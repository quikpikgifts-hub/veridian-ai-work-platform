    ...
    <div className="bg-[#08090F] border border-white/[0.07] border-t-amber-500 border-t-[2px] rounded-[3px] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-[12px] font-bold text-white/90">AI Agent Status</div>
            <div className="text-[10px] text-white/30 mt-0.5">{mockAIAgents.filter(a => a.status !== 'offline').length} agents operational</div>
          </div>
        </div>
        <a href="/ai-operations" className="text-[10px] text-amber-400/70 hover:text-amber-400 font-semibold transition-colors">Manage →</a>
      </div>
          ...
          return (
            <div key={agent.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[9.5px] text-white/28 truncate">{agent.lastAction}</div>
              </div>
              <div className="text-right flex-shrink-0">
                <div className="text-[11px] font-bold text-amber-400 font-mono">{agent.tasksToday}</div>
                <div className="text-[9px] text-white/25">tasks</div>
              </div>
            </div>
          );
        })}
